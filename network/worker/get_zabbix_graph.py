...
def get_image_by_host(ip):
    # 建立zabbix客户端
    zabbix_client = ZabbixAPI(url="http://122.51.39.93/zabbix")
    # 登录zabbix
    zabbix_client.login(user="Admin", password='zabbix')
    host_ids = 0
    # 获取host信息
    for host_info in zabbix_client.host.get(output="extend", monitored_hosts=1):
        if host_info['host'] == ip:
            host_ids = host_info['hostid']
            break
    # 根据host_id获取指定host的graph信息(下图中的10107是某一个host_id）
    graph_info = zabbix_client.graph.get(output='extend', hostids=host_ids)
