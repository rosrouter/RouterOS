...
def gettemplateid():
    ...
    resp = requests.post(settings.ZABBIX_URL, data=json.dumps(data), headers=settings.ZABBIX_HEADER).json()
...
def hostcreate(templateid, groupid, hosts):
    for host in hosts:
        data = {
            ...
            "auth": token,
            "id": 1}
        resp = requests.post(settings.ZABBIX_URL, data=json.dumps(data), headers=settings.ZABBIX_HEADER, timeout=100)
        print(resp.text)
